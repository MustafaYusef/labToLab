...
class TrackDriverScreen extends StatefulWidget {
  ...
  TrackDriverScreen(this.orderId, this.statusCode) {
    controller.orderId = orderId;
    // controller.getOrderGps();
  }
  @override
  State<TrackDriverScreen> createState() => MapScreenState();
}

class MapScreenState extends State<TrackDriverScreen> {
  ...
  moveCamera() async {
    ...
    try {
      ...
      final prof = await repo.getGpsOrder(token, widget.orderId);
      ...
      controller.moveCamera(CameraUpdate.newCameraPosition(CameraPosition(
          // bearing: 192.8334901395799,
          target: LatLng(
              double.parse(prof.order.lat), double.parse(prof.order.log)),
          tilt: 59.440717697143555,
          zoom: 16.151926040649414)));
      final Uint8List customMarker = await getBytesFromAsset(
          path: widget.statusCode == "العينة مع الناقل"
              ? "assets/images/shipment2.png"
              : "assets/images/shipment.png", //paste the custom image path
      ...
      setState(() {
        markers.add(Marker(
            markerId: MarkerId("1"),
            draggable: false,
            infoWindow: InfoWindow(title: "موقع السائق "),
